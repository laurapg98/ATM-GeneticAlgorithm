...
%% SECTION 4: GA loop
    ...
    MejorFit=[MejorFit, FitnessVector_Sorted(1,1)];
    MejorAffected=[MejorAffected BestAffectedConflict(end,1)];
    MejorConflicts=[MejorConflicts BestAffectedConflict(end,2)];
    end
    w
    MatrixFit=[MatrixFit; MejorFit];
    MatrixAffected=[MatrixAffected; MejorAffected];
    MatrixConflicts=[MatrixConflicts; MejorConflicts];
end
...
a=0:25:100;
figure
for i=1:1:size(MatrixFit,1)
    scatter(a,MatrixFit (i,:))
    hold on
end    
xlabel("Elitism (%)")
